...
- 검은 사각형 윤곽은 실제 출력 비율의 크롭 범위다. 윤곽은 흰 캔버스의 가로·세로 80% 안에 그리고 사방 10% 바깥 여백을 고정한다. 모델이 이 사각형을 캔버스 끝까지 확대해서는 안 된다. 캐릭터는 머리가 위 변, 발이 아래 변에 닿도록 사각형을 세로 100% 채우며 모든 픽셀은 내부에 있어야 한다.
