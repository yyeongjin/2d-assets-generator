...
export async function DELETE(_request: Request, context: { params: Promise<{ jobId: string }> }) {
  ...
  if (!/^[a-zA-Z0-9_-]{8,80}$/.test(jobId)) return NextResponse.json({ error: "잘못된 job_id입니다." }, { status: 400 });
  try {
    const response = await requestScail2(`/v1/jobs/${encodeURIComponent(jobId)}`, { method: "DELETE" });
    if (response.status === 204) return new Response(null, { status: 204 });
    const body = await responseJson(response);
    return NextResponse.json(body, { status: response.status });
  } catch (error) {
    const message = error instanceof Error ? error.message : "SCAIL-2 작업 삭제 실패";
    console.error("[SCAIL2][JOB_DELETE_ERROR]", { jobId, message });
    return NextResponse.json({ error: message }, { status: 502 });
  }
}
