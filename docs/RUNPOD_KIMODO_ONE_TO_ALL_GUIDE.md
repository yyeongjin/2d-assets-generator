# V9 RunPod Kimodo + One-to-All 설치/실행 가이드

> **상태: V9 스테이징 후보.** 사람 경로는 실제 캐릭터 종단 검증을 거친 V8 경로를 그대로 보존하고, 동물 요청만 `quadruped`와 `biped_animal` 전용 motion 경로로 분리한다. 코드·API 계약·오프라인 회귀 검사는 통과했지만 신규 동물 경로의 실제 L40S + One-to-All 종단 결과는 아직 프로덕션 승인 전이다. 이전 실패·성공 기록과 ZIP은 삭제하지 않고 `docs/`, `archives/`, `failures/`에 보존한다.

> 이 문서는 **아무 파일도 없는 새 RunPod Pod에서 시작하는 순서**로 작성한다. `install_v9_overlay.sh`는 먼저 V9 ZIP을 다운로드하고 압축을 푼 뒤에만 실행한다. ZIP이나 스크립트가 아직 없는 상태에서 `./scripts/...`를 먼저 실행하지 않는다.

모델·보조 체크포인트의 코드/가중치 라이선스와 재배포 조건은 [사용 모델·보조 체크포인트 라이선스 등록부](THIRD_PARTY_MODEL_LICENSES.md)에서 별도로 관리한다.
...
Job Queue
        ...
        │         V8 Human Adapter
        │
        ├─ quadruped | biped_animal
        │    └→ animal_procedural 또는 animal-motion-v1 NPZ
        │              │
        │              ▼
        │         V9 Animal Adapter
        │
...
- V9 ZIP을 다운로드하고 `/tmp/sprite_v9`에 압축 해제
- 압축 안의 `install_v9_overlay.sh`로 기존 `/workspace/sprite-pipeline`에 overlay 적용
...
- 사람 요청을 쓸 때 Motion worker `:9101`, 모든 요청에 One-to-All `:9102`, FastAPI `:8000` 실행
- 동물 전용 Pod라면 `START_KIMODO=false`로 Kimodo를 올리지 않음
...
- `subject_profile=character|quadruped|biped_animal|auto` 선택
- `motion_backend=kimodo|animal_procedural|animal_npz|auto` 선택
- 외부 동물 motion을 쓸 때만 `animal-motion-v1` NPZ 첨부
...
[sprite_pipeline_fastapi_full_v9.zip](https://github.com/yyeongjin/2d-assets-generator/blob/main/sprite_pipeline_fastapi_full_v9.zip)

V9 ZIP 자체는 수정하지 않고 RunPod에도 원본을 그대로 보관한다. 이전 V2·V3·V5·V6·V7 ZIP은 저장소의 [`archives/pipeline-versions/`](../archives/pipeline-versions/)에, V8 ZIP은 저장소 루트에 보존하며 현재 설치에는 사용하지 않는다. V4는 배포하지 않았다.
...
curl -L \
  "https://github.com/yyeongjin/2d-assets-generator/raw/refs/heads/main/sprite_pipeline_fastapi_full_v9.zip" \
  -o sprite_pipeline_fastapi_full_v9.zip
```

확인과 무결성 검사:

```bash
ls -lh /workspace/sprite_pipeline_fastapi_full_v9.zip
sha256sum /workspace/sprite_pipeline_fastapi_full_v9.zip
unzip -t /workspace/sprite_pipeline_fastapi_full_v9.zip
```

SHA-256은 다음 값과 정확히 같아야 한다.

```text
f4d655a6779dbaf091e53ab581c2c254b5e18e64459b2bd5fe0c105406a6d35e
```

마지막에 `No errors detected in compressed data`가 나와야 한다. 원본 ZIP은 `/workspace/sprite_pipeline_fastapi_full_v9.zip`에 그대로 둔다.

---

# 5. ZIP 압축 해제 후 V9 overlay 설치

이 단계에 도달하기 전에는 `./scripts/install_v9_overlay.sh`가 존재하지 않는다. 반드시 **4번에서 ZIP 다운로드와 무결성 검사를 먼저 끝낸 뒤** 아래 순서대로 실행한다.

`/workspace/sprite-pipeline/server/` 안에서 바로 압축을 풀지 않는다.

```bash
rm -rf /tmp/sprite_v9
mkdir -p /tmp/sprite_v9

unzip -q \
  /workspace/sprite_pipeline_fastapi_full_v9.zip \
  -d /tmp/sprite_v9
```
...
```bash
cd /tmp/sprite_v9/sprite_pipeline_fastapi_full_v9

./scripts/install_v9_overlay.sh \
...
```bash
cp -a \
  /tmp/sprite_v9/sprite_pipeline_fastapi_full_v9/server/requirements.txt \
  /workspace/sprite-pipeline/server/requirements.txt
```

설치 스크립트는 압축을 푼 V9 bundle을 기준으로 다음 코드와 문서만 갱신한다.
...
README_V9.md
MIGRATE_V8_TO_V9.md
ANIMAL_MOTION_NPZ_V1.md
RUNPOD_KIMODO_ONE_TO_ALL_GUIDE.md
V9_ANIMAL_ROUTING_SOURCE_AUDIT.md
.env.v9.example
V9_BUILD_REPORT.txt
V9_RELEASE_MANIFEST.json
```
...
grep -n -E "ADAPTER_VERSION|quadruped|biped_animal" adapter/animal_service.py
grep -n -E "PIPELINE_VERSION|subject_profile|motion_backend" server/app/pipeline.py
```

사람용 `WORKER_VERSION`과 `ADAPTER_VERSION`은 회귀 방지를 위해 `8.0.0`, 동물 adapter는 `9.0.0-animal`, 전체 `PIPELINE_VERSION`은 `9.0.0`이어야 한다.

## V9 코드 검사

One-to-All 서비스를 로드하기 전에 실행한다. V9 테스트에는 `workers/ota_worker.py`를 import하는 항목이 있어 `torch`와 `imageio`가 설치된 One-to-All venv를 사용한다. 새 Pod라면 먼저 이 문서의 **9. One-to-All venv 설치**까지 완료한 뒤 이 단계로 돌아온다.
...
전체 테스트가 `OK`로 끝나야 한다. 패키지 제작 시점 기준 Python 테스트는 66개이며, 숫자가 달라졌다면 `V9_BUILD_REPORT.txt`와 현재 테스트 목록을 함께 확인한다.

## V9 핵심 변경

사람·사람형 캐릭터는 아래 V8 동작을 그대로 사용한다.
...
동물 요청에만 다음 V9 분기를 추가한다.

- `subject_profile=quadruped|biped_animal`은 Kimodo와 사람 reference-pose detector를 우회
- `motion_backend=animal_procedural`은 profile별 닫힌 control cycle 사용
- `motion_backend=animal_npz`는 검증된 `animal-motion-v1` NPZ 사용
- 동물에 `kimodo`, 사람에 `animal_*`를 지정하면 queue 진입 전에 거부
- 사람 QC와 별도로 중복 개체, 인간화, 다리 수 붕괴, 몸통 분리, 정체성 변화와 loop 경계를 검사
- 실패 artifact도 `/v1/jobs/<JOB_ID>/debug`에서 내려받을 수 있도록 보존
...
> 라이선스 주의: `Wan-AI/Wan2.2-Animate-14B` 저장소는 Apache-2.0으로 표시되지만 이 파일의 원 프로젝트 YOLOv10은 AGPL-3.0이다. 정확한 파일별 권리가 확인되기 전에는 `yolov10m.onnx`를 저장소 ZIP, container image나 상용 비공개 배포본에 포함하지 않는다. 현재 판정과 교체 필요성은 [라이선스 등록부](THIRD_PARTY_MODEL_LICENSES.md)에 기록한다.
...
V9 overlay가 복사한 server requirements로 설치:
...
사람·캐릭터 요청을 받는 Pod는 아래 세션 1~3을 모두 실행한다. 동물 전용 Pod는 세션 1 Kimodo를 생략하고 세션 2 One-to-All과 세션 3 FastAPI만 실행한다.
...
# 세션 1 — Motion worker (`character` 요청에만 필요)
...
export ALLOW_UNAUTHENTICATED_API=false
export SPRITE_PIPELINE_ROOT=/workspace/sprite-pipeline
export SPRITE_JOBS_ROOT=/workspace/sprite-pipeline/jobs
export SPRITE_WORKER_READ_TIMEOUT_SECONDS=1800
export SPRITE_ANIMAL_MOTION_NPZ_MAX_BYTES=67108864
export SPRITE_ANIMAL_MOTION_NPZ_MAX_UNCOMPRESSED_BYTES=268435456
export SPRITE_ANIMAL_MOTION_NPZ_MAX_MEMBERS=64
...
export OTA_ANIMAL_FRONT_BACK_IMAGE_GUIDANCE_SCALE=3.25
export OTA_ANIMAL_FRONT_BACK_POSE_GUIDANCE_SCALE=1.05
export OTA_ANIMAL_SIDE_IMAGE_GUIDANCE_SCALE=3.35
export OTA_ANIMAL_SIDE_POSE_GUIDANCE_SCALE=1.15
export OTA_BIPED_ANIMAL_IMAGE_GUIDANCE_SCALE=3.10
export OTA_BIPED_ANIMAL_POSE_GUIDANCE_SCALE=1.20
export SPRITE_ANIMAL_QC_STRICT=true
...
동물 전용 Pod에서 Kimodo를 실행하지 않았다면 전체 `status`는 One-to-All 기준으로 `ready`, `character_ready`는 `false`, `animal_ready`는 `true`여야 한다.
...
## 18.2 사족동물 — 내장 procedural motion
...
curl --fail-with-body -X POST \
  ...
  -F "prompt=A four-legged animal walks naturally in place." \
  -F "seed=42"
```

## 18.3 동물 — 외부 `animal-motion-v1` NPZ

외부 client에 있는 `animal-motion-v1` NPZ를 첨부한다. V9 ZIP 안의 계약 fixture를 복사해 배선만 검사할 수도 있지만, 이 fixture는 학습 모델의 품질 표본이 아니다.

```bash
MOTION_NPZ=/외부/client/경로/quadruped_walk.npz
...
curl --fail-with-body -X POST \
  ...
  -F "motion_npz=@${MOTION_NPZ}" \
  -F "prompt=A four-legged animal walks naturally in place." \
  -F "seed=42"
```

NPZ를 RunPod에서 먼저 검사하려면 설치 대상에서 다음을 실행한다.

```bash
cd /workspace/sprite-pipeline

server/.venv/bin/python \
  scripts/validate_animal_motion_npz.py \
  /tmp/sprite_v9/sprite_pipeline_fastapi_full_v9/examples/animal-motion-v1/quadruped_walk_contract_example.npz \
  --subject-profile quadruped
```

계약의 필수 배열은 `front`, `back`, `left`, `right`이고 각각 `[T,18,2]`, `T=4..512`다. 좌표계는 `subject_box` 또는 `normalized_canvas`를 사용한다. 업로드 기본 제한은 압축 64 MiB, 압축 해제 총량 256 MiB, member 64개이며 NumPy object/pickle은 받지 않는다. 전체 필드와 metadata 규칙은 V9 ZIP을 설치하면 생성되는 `/workspace/sprite-pipeline/ANIMAL_MOTION_NPZ_V1.md`에서 확인한다.
...
queued
↓
animal_procedural_adapter 또는 animal_npz_adapter
↓
render_front_attempt_1 ... render_right_attempt_1
↓
appearance_attempt_1
↓
loop_qc_attempt_1
...
동물 identity/topology QC가 실패해도 artifact를 삭제하지 않는다. 외부 client에서 다음 endpoint로 `failure_debug.zip`을 내려받아 입력, 렌더 attempt와 QC 사유를 확인한다.

```bash
curl -L \
  "https://${POD_ID}-8000.proxy.runpod.net/v1/jobs/${JOB_ID}/debug" \
  -H "Authorization: Bearer ${API_TOKEN}" \
  -o failure_debug.zip
```
...
이 문서는 V3 실패 motion 분석, V5 공식 fixture 종단 성공, V6 cardinal 보정, V7 loop 안정화, V8 사람 경로의 appearance·최종 PNG QC와 V9 동물 routing을 모두 반영한 **현재 단일 설치·운영 가이드**입니다.
