...
아래는 오른쪽 측면 제자리 걷기 영상에서 추출한 실제 프레임 시트다. 일부 프레임에는 다리 교대가 보이지만, 같은 자세가 반복되고 신체 형태와 움직임 간격이 일정하지 않아 스프라이트 주기로 채택하지 않았다.

![Wan2.2 오른쪽 걷기 영상에서 추출한 프레임 시트](assets/wan-video/right-walk-sampling-frames.png)

같은 결과를 시간 순서로 확인한 GIF다. 정지에 가까운 구간과 불균일한 동작 변화가 함께 보인다.

![Wan2.2 오른쪽 걷기 영상 추출 프레임 GIF](assets/wan-video/right-walk-sampling.gif)
