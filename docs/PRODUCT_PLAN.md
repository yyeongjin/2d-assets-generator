...
예정 구조의 첫 목표는 방향별 17프레임에서 8프레임을 추출해 32 PNG를 만드는 것이다. 실제 출력이 반복 가능한 closed cycle인지 확인한 뒤 추출 index를 결정한다.
