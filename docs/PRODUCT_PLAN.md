...
농장 생활 RPG에 필요한 수천 개의 2D 에셋을 생성하고 검수하는 로컬 도구를 만든다. 이미지 모델과 영상 모델은 RunPod에서 실행한다. 로컬 화면은 옵션 입력, 모델 요청, 원본 결과 비교, 승인, 히스토리, 이후 Unity 검증을 담당한다.
...
이미지 단계에서 4방향과 장비 일관성이 확인된 뒤 연결한다.
...
왼쪽 동작은 오른쪽 결과를 반전해도 문제가 없는 캐릭터와 장비에서만 반전한다. 좌우 비대칭 의상이나 장비는 왼쪽을 별도로 생성한다.
...
4. 사용할 프레임 승인
5. 사용자가 후처리 실행
6. 배경 제거, 공통 바닥선 정렬, 규격 리사이즈
7. 스프라이트 시트와 Unity manifest 생성

생성 요청이 끝날 때마다 자동 후처리하지 않는다.
...
5. 방향별 영상 모델 연결과 동작 프레임 추출
6. 전 프레임 승인 뒤 수동 후처리와 스프라이트 시트 생성
7. 같은 구조를 타일·오브젝트·생명체·아이템 카탈로그로 확장
8. Unity import·slice·pivot·animation 검증 연결
