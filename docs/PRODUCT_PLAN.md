...
| 방향 기준 이미지 미승인 | 정면 중립 기준으로 선택 방향의 무동작 기준 이미지부터 생성하고 승인 |
| 방향 기준 이미지 승인 | 선택한 방향의 7개 동작 옵션 활성화, 같은 방향 승인 이미지를 Reference A로 자동 연결 |
...
| 입력 | 필수 | 내용 |
|---|---:|---|
| Reference A | 예 | 캐릭터 동작은 선택 방향과 같은 방향의 승인 기준 이미지. 방향 기준을 처음 만들 때만 도구 1의 정면 중립 기준 이미지 사용 |
...
1. 도구 1에서 승인한 정면 중립 이미지를 이용해 `front.ref`, `back.ref`, `left.ref`, `right.ref` 무동작 기준을 생성·승인한다.
2. 동작을 생성할 때는 선택한 방향과 같은 방향의 승인 기준을 Reference A로 사용하고, 같은 방향·동작의 포즈 가이드를 Reference B로 사용한다.

방향 기준이 없으면 다른 방향 동작을 바로 생성하지 않는다. 예를 들어 `left/tool_swing`은 `left.ref`가 승인된 뒤에 생성한다.
...
- 기본 가이드에서 프레임은 흰 캔버스의 가로·세로 80% 안에 두고 사방 10% 바깥 여백을 고정한다. 프레임과 흰 캔버스 바깥선은 서로 다른 경계다.
- 생성 결과가 프레임을 캔버스 바깥선까지 확대해 해석하면 실패로 기록하며, 그 결과에 맞춰 프레임 규격을 키우지 않는다.
...
- 생성 대상, 장비, 운반 물체, 동작 궤적은 모두 윤곽선 안에 있어야 하며 윤곽선 밖 픽셀은 허용하지 않는다.
- 같은 에셋의 방향·상태·동작 변형은 같은 ratio와 바닥선을 재사용한다.
- 같은 ratio를 쓰더라도 방향별 내부 점유 규격은 같지 않다. 정면·후면은 어깨와 양팔이 보이는 넓은 점유를, 좌측·우측은 몸통 폭을 줄이고 팔다리가 겹치는 좁은 점유를 사용한다.
...
| `direction_overrides` | 바깥 rect는 유지하면서 방향별 내부 점유 폭, 좌우 safe margin, 관절·장비 anchor를 조정하는 값 |
...
방향별 layout template은 같은 이미지가 아니다.

| 방향 | 내부 점유 기준 | 가이드 차이 |
|---|---:|---|
| `front` | 가로 약 78% | 얼굴, 양쪽 어깨·팔·발 표시 |
| `back` | 가로 약 76% | 얼굴 표시 제거, 후두부·등 실루엣 표시 |
| `left` | 가로 약 56% | 몸통 폭 축소, 원근상 먼 팔·다리는 약하게 겹침 |
| `right` | 가로 약 56% | 좌측 기본을 반전하되 비대칭 장비 anchor는 별도 저장 |

위 점유율은 초기 `character.standard` 가이드 값이며 캐릭터 family별 revision으로 조정할 수 있다. 바깥 rect와 공통 바닥선은 atlas 정렬을 위해 유지한다.
...
| 방향·동작 생성 | 정면 기준에서 4방향 무동작 기준을 먼저 승인하고, 같은 방향 기준으로 7개 동작 선택·I2I 실행 |
