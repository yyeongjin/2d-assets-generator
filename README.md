...
> 현재 단계: 제품 기획 및 에셋 카탈로그 설계. 아직 실행 가능한 생성기는 구현되지 않았습니다.
...
승인된 기준 캐릭터나 오브젝트를 입력하면 정체성과 스타일을 유지하면서 방향, 상태, 동작 프레임을 생성합니다.

- 입력 1: 기준 캐릭터/오브젝트 이미지
- 입력 2: 흰 배경, 검은 형태로 구성한 레이아웃·포즈 가이드
- 입력 3(선택): 무기, 도구, 의상 또는 스타일 참조 이미지
- 방향: 정면, 후면, 좌측, 우측
- 캐릭터 동작: 대기, 빈손 걷기, 한손 무기, 양손 무기, 도구 휘두르기, 활 쏘기, 물건을 앞에 들기, 물건을 머리 위에 들기
...
- 실행 환경: RunPod의 NVIDIA GPU Pod
- 모델: `Qwen/Qwen-Image`, `Qwen/Qwen-Image-Edit-2509`
- 추론: Python, PyTorch, Hugging Face Diffusers
- MVP UI: Gradio
- 이미지 처리: Pillow, NumPy; 필요 시 배경 제거/팔레트 도구 추가
- 데이터: 프로젝트 manifest(JSON/YAML), 생성 이력(JSONL 또는 SQLite)
- Unity 연동: PNG + JSON manifest를 시작으로, 이후 Editor importer 패키지 제공
...
1. **M0 — 생성 실험대:** RunPod에서 T2I/I2I 한 장 생성, 설정 및 결과 저장
2. **M1 — 기준 에셋 생성기:** 카테고리별 동적 폼, 프롬프트 빌더, 후보 승인
3. **M2 — 캐릭터 스프라이트:** 4방향과 핵심 동작, 가이드 입력, 시트 패킹
4. **M3 — 타일·오브젝트 파이프라인:** 자동 타일 규칙, 상태/계절 변형, 충돌·pivot 메타데이터
5. **M4 — Unity 디버거:** 자동 import, slice, 애니메이션 클립, 테스트 씬 검증
6. **M5 — 대량 생산:** 카탈로그 기반 배치 큐, 실패 재시도, 품질 리포트, 비용/시간 추적

세부 범위, 화면 설계, 데이터 구조, 완료 조건은 제품 기획서에 기록합니다.
