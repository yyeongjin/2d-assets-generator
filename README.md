...
캐릭터 네 방향은 이 검은 사각형의 위·아래 변을 공통 높이 기준으로 사용합니다. Reference A의 흰 여백을 먼저 정규화하고, Reference B에는 머리가 위 변에 닿고 발이 아래 변에 닿는 포즈를 표시합니다. 정면·후면·좌측·우측의 세로 높이는 같고 방향에 따라 가로 점유 폭만 달라집니다.
...
후면과 좌측은 정면 기준으로 생성하고 우측은 좌측 결과를 수평 반전해 좌우 일관성을 확보합니다. 배치가 중단되면 이미 저장된 PNG/JSON을 건너뛰고 실패 프레임부터 재개하며, 일시적 endpoint 오류는 자동 재시도합니다.
